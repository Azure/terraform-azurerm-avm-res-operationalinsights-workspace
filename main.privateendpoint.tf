...
resource "azurerm_private_endpoint" "this" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azapi_resource.amplscope[each.value.pl_key].id
  ...
  dynamic "ip_configuration" {
    ...
    content {
      ...
      member_name        = "each.value.azuremonitor" # map to each.value.subresource_name if there are multiple services.
      subresource_name   = "each.value.azuremonitor" # map to each.value.subresource_name if there are multiple services.
    }
  }
